...
@Component({
  selector: 'app-summaries-health-page',
  template: `
...
export class SummariesHealthPageComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {

  public subscriptions: Subscription[] = [];

  // All state transfer objects.
  @select() values_health: Observable<any>;
  @select() summaries_health: Observable<any>;
  public theSummariesHealthByName: any;
  public valueNames: any[];
  // @Input getter/setter variables... see below.
  private _active = false;
  private _filterText = null;
  private _toggleButtonBar = false;
  // keep track of the grid.QALY_RF
  public columnID = 0;
  public totalRows = 0;
  ...
  public subscribeToData() {
    this.subscriptions.push(
      this.summaries_health.subscribe(
        rowData => {
  ...
  ngOnInit() {
    // Grab the state data.
    this.subscriptions.push(
      this.summaries_health.subscribe(value => {
        this.summarieshealthHandler(value);
      })
    );
    this.subscriptions.push(
  ...
  private summarieshealthHandler (value) {
    // save the 'summaries_health name' data.
    this.theSummariesHealthByName = JSON.parse(JSON.stringify(value)).map(d => d.outcome);
  }
  ...
  public getNameDuplicate(newName) {
    return this.theSummariesHealthByName.filter(name => name === newName).length > 0;
  }
  ...
  public addThisRow(groupname) {
    // Add summaries health...
    this.ngRedux.dispatch({
      type: 'ADD_SUMMARIES_HEALTH',
      payload: this.createNewRowData(groupname)
    });
  }
  
  // reducer - add a new group...
  public addSummariesHealth() {
    // Add summaries health...
    this.ngRedux.dispatch({
      type: 'ADD_SUMMARIES_HEALTH',
  ...
  public insertSummariesHealth(insert) {
    // Insert state...
    this.ngRedux.dispatch({
      type: 'INSERT_SUMMARIES_HEALTH',
      payload: [
  ...
  public deleteSummariesHealth(stateid) {
    this.ngRedux.dispatch({
      type: 'REMOVE_SUMMARIES_HEALTH',
      payload: stateid
    });
  }
  ...
  public onInsertRowBeforeSelection() {
    ...
    this.selectedRow = selected[0].rowIndex;
    // tell the reducer...
    this.insertSummariesHealth(index);
  }
  ...
  public onInsertRowAfterSelection() {
    ...
    this.selectedRow = selected[0].rowIndex;
    // tell the reducer...
    this.insertSummariesHealth(index + 1);
  }
  ...
  public onRemoveSelected($event) {
    ...
    if (typeof selectedData[0] !== 'undefined' && selectedData[0] !== null) {
      ...
      // tell the reducer...
      this.deleteSummariesHealth(selectedData['0'].id)
      // show'em all again...
      this.gridOptions.api.forEachNode(function(node) {
