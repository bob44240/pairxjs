...
@Component({
  selector: 'app-values-health-page',
  template: `
    <app-formula-bar [name]="'valueshealthFB'"></app-formula-bar>
...
export class ValuesHealthPageComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {

  public subscriptions: Subscription[] = [];

  // All state transfer objects.
  @select() values_health: Observable<any>;
  public theValuesHealthByName: any;
  ...
  ngOnInit() {
    // Grab the state data.
    this.subscriptions.push(
      this.values_health.subscribe(value => {
        this.valueshealthHandler(value);
      })
    );
    // setup stuff locally.
    this.subscriptions.push(
  ...
  private valueshealthHandler (value) {
    // save the 'values_health name' data.
    this.theValuesHealthByName = JSON.parse(JSON.stringify(value)).map(d => d.name);
  }
  ...
  public getNameDuplicate(newName) {
    return this.theValuesHealthByName.filter(name => name === newName).length > 0;
  }
  ...
  public insertValuesHealth(insert) {
    // Insert state...
    this.ngRedux.dispatch({
      type: 'INSERT_VALUES_HEALTH',
      payload:
  ...
  public deleteValuesHealth(stateid) {
    this.ngRedux.dispatch({
      type: 'REMOVE_VALUES_HEALTH',
      payload: stateid
    });
  }
    ...
    return [
      {
        ...
        cellRenderer: function (params) {
          return '<div class="datatype datatype-pill datatype-value-primary">' + params.value + '</div>';
        },
      ...
      {
        headerName: '<p style="text-align: right;">Name</p>',
        headerTextName: '',
        field: 'spacer',
        colId: (this.columnID++).toString(),
        width: 120,
  ...
  public onInsertRowAfterSelection() {
    ...
    this.selectedRow = selected[0].rowIndex;
    // tell the reducer...
    this.insertValuesHealth(index + 1);
  }
  ...
  public onRemoveSelected($event) {
    ...
    if (typeof selectedData[0] !== 'undefined' && selectedData[0] !== null) {
      ...
      // tell the reducer...
      this.deleteValuesHealth(selectedData['0'].id)
      // show'em all again...
      this.gridOptions.api.forEachNode(function(node) {
