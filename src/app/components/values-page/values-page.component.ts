...
@Component({
  ...
  <div style="background-color: #ffffff">
    ...
    <md-tab-group id="valuesHiddenTab"
                  class="heRo-value-tab"
                  [(selectedIndex)]="index"
                  (selectChange)="selectionChange($event)">
      <md-tab label="Health">
        <app-values-health-page #health [active]="_active && index == 0"
                                        [filterText]="inputTextHealth"
                                        [toggleButtonBar]="toggleButtonBarHealth">
        </app-values-health-page>
      </md-tab>
      <md-tab label="Economic">
        <app-values-economic-page #economic [active]="_active && index == 1"
                                          [filterText]="inputTextEconomic"
                                          [toggleButtonBar]="toggleButtonBarEconomic">
        </app-values-economic-page>
      </md-tab>
    </md-tab-group>
  </div>
  `,
...
export class ValuesPageComponent {
  ...
  set active(value: boolean) {
    ...
    if (this.active && !this.init) {
      $('#valuesHiddenTab > .mat-tab-header').hide();
      this.init = true;
    }
    if (this._active) {
